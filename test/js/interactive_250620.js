$(document).ready(function(){
    ...
    let photo_end
    let photo_scroll
    let photo_w_start = 40 //시작할때 넓이
    let photo_w_end = 100 //종료할때 넓이
    let photo_w

    function photo_resize(){
        photo_top = photo.offset().top
        photo_start = photo_top - (win_h * 0.8)
    ...
    $(window).scroll(function(){
        scroll_chk() //스크롤 할 때 마다 1번씩
        slogan_ani()
        dark_bg()
        photo_resize()
    })
    $(window).resize(function(){
        slogan_ani() //브라우저가 리사이즈 될 때 마다 1번씩 실행
        resize_chk()
        dark_bg()
        photo_resize()
    })
       ...
       // console.log(slogan_scroll)
        ...
        if(slogan_start > scrolling){
            ...
            for(i=0; i<slogan_leng; i++){
                ...
                if(slogan_w > 100){
                    slogan_w = 100 //span이 100% 이상 넘는 걸 막음
                }
