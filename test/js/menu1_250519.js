$(document).ready(function(){
    ...
    /* 
        메뉴에 마우스를 오버하면
        메뉴 : header.menu_over .gnb .gnb_wrap ul.depth1 > li
        1. header에 menu_over라는 클래스 추가
            header addClass
    ...
    */
    ...
    $('header .gnb .gnb_wrap ul.depth1 > li').on('mouseenter focusin', function(){
        if(device_status == 'pc'){ //pc일때만 실행
            ...
            ----> 원래 over 클래스가 없던 요소는 removeClass줘도 아무일도 안일어남 */
            $('header .gnb .gnb_wrap ul.depth1 > li').removeClass('over')
            $(this).addClass('over') //위에 $와 순서가 바뀌면 removeClass가 제일 마지막에 오니까 over가 안나타남
        }//if
    })
    ...
    /* 
        ...
        -----> 메뉴를 클릭했을때
                ...
                hasClass -> class가 있는지 여부를 확인(true/false 반환)

        4. 모바일에서 1차 메뉴를 클릭하면 1차 메뉴 a에 입력되어 있는 링크주소로 이동
            메뉴를 안열고 ...
            a태그에 있는 href로 페이지 이동되는 것을 막아야함 ...
            a에 클릭했을 때로 선택자 변경
